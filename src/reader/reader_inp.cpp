  ...
  while(std::getline(ifs, str, ';'))
    {
      ...
      if(str[0]=='*'){// begin card
        ...
        mode = push_param(&str);
        mode = std::string::toupper(mode);
        clean_string(&mode, ' ');

        std::vector<string> param_tmp = split_string(&str, '.');
        
        for(std::string iparam: param_tmp){
          std::vector<string> s = split_string(iparam, '=');
          params[std::string::toupper(s[0])]=s[1];
        }
      }else{
        if(mode=="*NODE"){
          std::vector<string> values = split_string(&str);
          if(values.size()<4){
            std::cout << "syntax error at *node card";
            exit(-1);
          }
          int nn = stoi(values[0]);
          double x = stod(values[1]);
          double y = stod(values[2]);
          double z = stod(values[3]);
          dcovec3 x_0(x, y, z);
          ...
          int en = stoi(values[0]);
          std::vector<int> connectivity(4);
          for(int i=1;i<=4;i++){
            connectivity[i] = stoi(values[i+1]);
          }
